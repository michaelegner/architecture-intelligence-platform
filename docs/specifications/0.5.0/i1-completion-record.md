...
Per §5.3 ("Each mapping entry retains its stable artifact identity, revision, content digest,
attribution, normalized source pointers, targets, and semantic options"), the mapping-context
digest also carries this artifact's `contentDigest` (SHA-256 of the file's own exact raw bytes,
mirroring §5.2's `content_sha256` convention) and `attribution` (the configured file's own
*basename*, not its full path - §5.3 also requires "Capture times and physical checkout paths are
excluded", and the full configured path is exactly such a physical, checkout-root-dependent value)
- there is no per-artifact "semantic options" concept this mechanism exposes, so that part of
§5.3's list has nothing to project here.
...
| `uv run pytest tests/unit` | 1372 passed |
...
| Mapping-context digest reflects real configured/manifest/shared/migration mappings (including each entry's own documentPath, contentDigest, and basename-only attribution per §5.3) and adapter versions; unrelated context change triggers reevaluation without spurious revision advance; attribution stays portable across checkout roots *(4 for shared/migration content)* | `tests/unit/test_orchestrator.py::test_a_real_migration_mapping_changes_the_mapping_context_digest`, `::test_a_document_path_only_change_still_changes_the_mapping_context_digest`, `::test_a_content_digest_only_change_still_changes_the_mapping_context_digest`, `::test_an_attribution_only_change_still_changes_the_mapping_context_digest`, `::test_attribution_is_portable_across_two_absolute_checkout_roots` |
