...
mirroring §5.2's `content_sha256` convention) and `attribution` (the configured file's own path) -
there is no per-artifact "semantic options" concept this mechanism exposes, so that part of §5.3's
list has nothing to project here.
...
| `uv run pytest tests/unit` | 1371 passed |
...
> `uv run pytest tests/unit` (1371) and `tests/integration` (300) both pass in full; lint/format are
