...
A1–A5, `async_flow_to` and O1–O4 remain **Queue-only**. They traverse `SENDS -> Queue <-
RECEIVES_FROM` and never follow v0.5.0 I4's `PUBLISHES_TO`/`SUBSCRIPTION_OF` Pub/Sub topology. Pub/Sub
dependencies and drift are answered only by the Architecture Intelligence tools (see
[`mcp.md`](mcp.md#pubsub-in-dependency-and-drift-answers-v050-i4)), plus O5's widened
messaging-coverage signal below.
...
| O5 | `GET /api/analysis/runtime/coverage` | `telemetry_coverage` | Per-service telemetry coverage (`http_observed`/`messaging_observed`/`spans_observed`) — used to judge how much weight an O4 finding should carry. Since v0.5.0 I4, `messaging_observed` also counts observed `PUBLISHES_TO -> Topic` and `RECEIVES_FROM -> Subscription` |
