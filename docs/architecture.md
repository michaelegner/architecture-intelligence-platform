...
| `architecture_intelligence.py` | `/api/services/{id}/dependencies`, `/api/services/{id}/drift`, `/api/services/{id}/deployments` | REST parity for `ArchitectureIntelligenceService.get_service_dependencies`/`.get_architecture_drift` (v0.5.0 I3 slice 5a); `/deployments` is a REST-only bounded projection over `get_service_dependencies`'s deployment data, with required `environment`/`from`/`to` and a real `404` for an unknown Service (v0.5.0 I3 slice 5b). The underlying `Service -[DEPLOYED_AS]-> Workload` identity — reconciled from an explicit annotation, a configured mapping, or a qualified OpenTelemetry/Kubernetes linkage — is documented in full in [`graph-model.md`](graph-model.md#deployed_as-v050-i3--computed-not-a-stored-graph-edge) and [`mcp.md`](mcp.md#deployed_as-deployment-claims-v050-i3). Since v0.5.0 I4, dependency/drift claims also carry declared Topic/Subscription routes — see [`mcp.md`](mcp.md#pubsub-in-dependency-and-drift-answers-v050-i4) |
